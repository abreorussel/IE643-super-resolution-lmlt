...
def calculate_lpips(img, img2, net='vgg'):
    ...
    normalize(img2_tensor, [0.5, 0.5, 0.5], [0.5, 0.5, 0.5], inplace=True)
...
def main(args):
    ...
    for i, img_path in enumerate(img_list_gt):
        ...
        # Calculate LPIPS
        lpips_value = calculate_lpips(img_gt, img_restored)

        logger.info(f'{i+1:3d}: {basename:25}. \tPSNR: {psnr:.6f} dB, \tSSIM: {ssim:.6f}, \tLPIPS: {lpips_value:.6f}')
